...
class PregateInHome extends State<PregateIn> with TxtCss {
  ...
  Widget build(context) {
    ...
    return MaterialApp(
      title: 'Pre Gate-In',
      home:Scaffold(
        appBar: AppBar(title: Text('Container PreGate-In',style: txtRoboBoldHiLightColor(30.0, Colors.white)), centerTitle: true,
                      actions: <Widget>[
          ...
          children: <Widget>[
            Row(
              ...
              SingleChildScrollView(
                ...
                Container(
                  //width: 400.0,
                  width:((31.25/100) * sizeHW.width), //31.25% of original width
          ...
          rows: damageCodesList.map((damagecode) => DataRow(
                ...
                cells: <DataCell>[
                  ...
                  DataCell(SizedBox.fromSize(size: Size(70, 50),
                            child: Image.asset(damagecode.imageSource, alignment: Alignment.centerRight,),),
                            onTap: (){pgi.showImage(context, bloc,damagecode);}) ,
                  DataCell(SizedBox.fromSize(size: Size(50, 50),
                            child: IconButton(icon: Icon(Icons.delete), iconSize: 30, onPressed: (){pgi.deleteCodes(bloc, damagecode);}))),
                ]
          )).toList()
        );
